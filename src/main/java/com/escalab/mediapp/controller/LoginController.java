...
import com.escalab.mediapp.util.EmailUtil;
import com.escalab.mediapp.util.Mail;
...
public class LoginController {
    ...
    public ResponseEntity<Integer> enviarCorreo(@RequestBody String correo) {
        ...
        try {
            ...
            if (us != null && us.getIdUsuario() > 0) {
                ...
                mail.setTo(us.getNombreUsuario());
                mail.setSubject("RESTABLECER CONTRASEÑA - MEDIAPP");

                Map<String, Object> model = new HashMap<>();
                //http://localhost:4200/recuperar/123e4567-e89b-12d3-a456-426655440000
                String url = "http://localhost:4200/recuperar/" + token.getToken();
                model.put("user", us.getNombreUsuario());
            ...
            if (token != null && !token.isEmpty()) {
                ...
                if (rt != null && rt.getId() > 0) {
                    if (!rt.estaExpirado()) {
                        ...
                        rt.setCount(rt.getCount() + 1);
                        tokenService.update(rt);
                    }
                }else{
                    rt.setCount(rt.getCount() + 1);
                    tokenService.update(rt);
    ...
    public ResponseEntity<Integer> restablecerClave(@PathVariable("token") String token, @RequestBody String clave ) {
        ...
        try {
            ...
            rpta = service.cambiarClave(claveHash, rt.getUser().getNombreUsuario());
